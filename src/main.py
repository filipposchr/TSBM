...
class MLPWithPTD(nn.Module):
    # MLP integrating pass-through degree feature (ptd_feat)
    # Inputs: ptd_feat: pass-through degree feature of 1-dim (scalal per node)
    #         src_feat (default feature of 128-dim)
  
    # 1. ptd_proj: project ptd_feat to 128-dim
    # 2. concatenating the two features (128 + 128 = 256-dim)
    # 3. Reduce dim of combined features to final_dim=128 through self.input_proj

    def __init__(self, node_dim=128, ptd_dim=1, ptd_proj_dim=128, final_dim=128, drop=0.1):
        super().__init__()

        # Project scalar PTD to match node feature space
        self.ptd_proj = nn.Sequential(
            nn.Linear(ptd_dim, ptd_proj_dim),
            nn.ReLU(),
            nn.Dropout(drop)
        )

        # Combine projected PTD + src_feat → final_dim
        self.input_proj = nn.Sequential(
            nn.Linear(node_dim + ptd_proj_dim, final_dim),
            nn.ReLU(),
            nn.Dropout(drop)
        )
    ...
    def forward(self, src_feat, ptd_feat):
        if ptd_feat.dim() == 1:
            ptd_feat = ptd_feat.unsqueeze(-1)  # [B, 1]

        ptd_proj = self.ptd_proj(ptd_feat)  # [B, 128]
        x = torch.cat([src_feat, ptd_proj], dim=1)  # [B, 256]
...
MLP_model = MLPWithPTD().to(device)
